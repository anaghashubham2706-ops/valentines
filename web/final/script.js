...
var starArray = [];
/* ===============================
   ADDED STATE + MUSIC + VIDEO
   =============================== */
let started = false;
let startTime = null;
let showVideo = false;

const bgMusic = new Audio("public/Blue.mp3");
bgMusic.loop = true;
bgMusic.volume = 0.6;

/* Create video element */
const video = document.createElement("video");
video.src = "public/video/love.mp4";
...
startButton.style.textShadow = "0 0 15px rgba(180,180,255,1)";
...
function drawText() {
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much ANAGHA , more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much ANAGHA , more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
...
function draw() {
  ...
  if (started) {
    const elapsed = (performance.now() - startTime) / 6000;
